...
export const PROHIBITIONS: Prohibition[] = [
  ...
  // 同人/衍生模式：人物、关系、支线、CP 均可自由虚构与二次创作，只保留版权合法性红线。
  // （原「只能围绕原著真实人物/情节」的保真铁律已放开，以支持如「孙悟空×紫霞」等原著/影视衍生的角色情感线。）
  { tier: "L1", text: "本作为同人/衍生二次创作：可自由虚构原著/影视里的人物关系、恋爱线、支线与走向（含跨作品、影视版CP、原著未写明的情感），鼓励大胆演绎；唯一红线是不得抄录受版权保护作品的成段原文、不得声称这是官方正史" },
  {
    tier: "L1",
  ...
  // —— L2 支柱级 ——
  // 同人模式：角色可以做原著里没做过的事（如谈一段原著没写的恋爱），但仍要贴合这个角色「会怎么爱/怎么反应」的性格底色，别写成完全不认识的另一个人。
  { tier: "L2", text: "同人演绎可以让角色进入原著没有的情节与关系，但要守住角色的性格内核与说话方式——放飞剧情，别放飞人设" },
...
export function buildProhibitionsBlock(intensifyMode = false): string {
  ...
  // 保真约束已整体放开为同人/衍生模式，故不再需要按 intensifyMode 单独软化某条铁律。
  const lines = sorted.map((p) => p.text);
