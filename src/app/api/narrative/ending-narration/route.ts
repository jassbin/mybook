...
第二段（选择模式镜像，40-60字，只写一段给"你"看的话）：直接对话"你"，点出模式规律。参考：「你${patternWord}」「你${reversalWord}」。让玩家感受到"这说的是我"。不评判对错。${lifeLine ? "结尾可轻轻把他一生的命题（伤口/守护/怕失去）折向玩家自己——不点破他，而是让玩家意识到『他一生在追问的，也正是你要面对的』。" : ""}
【硬性要求】这一段只能是自然语句，绝对不许出现任何格式说明、题目、分隔符（如"===SEP==="）、"严格按格式""输出""JSON""数组"等字样；写满这一段后立刻停下，把后面几段的内容留到对应段落再写。
