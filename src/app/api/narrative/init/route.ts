...
import { getThrillConfig, PERSONA_AXES, THRILL_STRATEGIES } from "@/lib/agent/thrill";

export async function POST(request: NextRequest) {
  try {
    const { bookTitle, character, intensify, normalChoiceHistory, characterDomains, themeDomains, channel } = await request.json() as {
      bookTitle: string;
      character?: string;
      intensify?: boolean;
...
export async function callActGenerator({
  ...
  const intensifyBlock = intensifyMode
    ? `\n【极压模式——全程生效·情节与选项双重极限】
① 情节层（正文必须主动往极限推）：本幕的故事情节本身就要往绝境推进，不许温吞平铺、不许停留在铺垫或日常。要在本幕内把角色实打实逼到一个不可回头的临界关头——生死、决裂、背叛、亲手毁掉所珍视之物这一类真正无法挽回的关口，让玩家读正文时就感到"退无可退、必须做出可怕的抉择"。
② 因为本局已放开原著命运（平行推演），情节可以、也鼓励走向原著里没有的、更极端的转折与后果——前提是由玩家已做的选择+前文伏笔合理推导、代价看得见、因果写清楚，绝不天降。
③ 选项层：每幕困境直接是生死/无法挽回的抉择，没有「先忍忍」、没有软着陆，三个选项的代价都是永久的，只是方向不同。`
    : "";
...
　【严禁"先答应后反悔"的无因果跳变】绝对不许出现"对方先说『遵命/好/我留下』、下一句却又走了/又变卦了"这种没有原因的翻转。若对方最终仍走向既定结局，必须在同一段后果里、在他改变之前就写清是什么具体力量（军令、他人逼迫、他自己想通、形势突变）让他不得不收回那句承诺——先给因、再有果，读者要能看懂"他为什么答应了又走"。`;
...
· 让它随剧情推进由隐到显，最终由玩家自己回味，而不是被告知。`
  ...
  // 极限模式·正文段专用施压（只在极限模式出现，确保正文情节本身往绝境走，而不是只让选项狠）
  const sceneExtremeReq = intensifyMode
    ? "\n- 【极限·情节必须推到绝境】本幕正文不许温吞铺垫，要在本幕内把角色实打实逼到一个不可回头的临界关头（生死/决裂/背叛/亲手毁掉所珍视之物）；本局已放开原著命运，允许写出原著没有的、更极端的转折，但必须由玩家已做的选择+前文伏笔推导、代价看得见、因果写清。让玩家读完正文就感到退无可退。"
    : "";
...
【本幕开场钩子】${openingHook ?? "续接上一幕的张力，不要另起炉灶"}`;
  ...
  · 若本幕是【打脸/逆袭/识破/被偏爱/心动/扬眉吐气之类的爽点时机】→ actKind="thrill"：三个选项是**三种不同的"爽法"**（不是代价梯度！），每个 choiceKind="thrill" 并标 strategy：
    ...
    · 【爽点幕正文必须先抑后扬】爽点幕的正文（messages）开头要先把"憋屈"垫足——具体写出是谁在轻视/羞辱/为难"你"，让读者先替你不平（一两句即可），再把角色推到可以翻盘的那一刻。没垫憋屈的爽点不甜，必须先抑。
    · 【关键·爽点兑现要"三拍"·最重要】choiceKind=thrill 的选项，consequenceMap 里对应的后果**必须写成一段解气的"爽点兑现"**，且严格按三拍来写，绝不允许只写一句"对方脸色一变"这种敷衍反馈：
      ① 第一拍【你的动作】：具体写出"你"因这个选择做了什么、说了哪句话（要点出是"你"主动赢的，主角感拉满，例如"你不慌不忙抬眼，一句话戳破他的谎"）。
      ② 第二拍【对方狼狈】：把被打脸那一方的狼狈写细、写具体——不是"脸色一变"，而是"他后退半步，扶着桌角，那句准备好的嘲讽卡在喉咙里，脸涨得通红"。
      ③ 第三拍【全场反应/名场面收尾】：写周围人的哗然、倒吸气、刮目相看，或对方的彻底哑火，把高光钉死，让读者读完脑子里"嗡"一下，忍不住"就是这样！"。
      · 长度约 2-4 句，节奏要爽脆，动词要狠，绝不写成代价、失去或犹豫。被偏爱/心动类的爽点同理三拍：你做了什么→对方为你破例/失控/低头→旁人惊叹或你心头一颤。
...
- 【名场面 climaxScene】爽点幕的爽点爆发时机，尽量给出 climaxScene 高光文本（把三拍里最炸的那一下再放大成 2-3 句的名场面）；两难幕通常留空。`;
  const climaxFieldJson = `,\n  "actKind":"<dilemma | thrill 本幕类型>",\n  "climaxScene":{"title":"名场面标题（6字内，如「十里红妆」「一剑封喉」）","text":"名场面高光正文（2-3句，把爽点拉满，动词要狠、画面要炸：打脸/逆袭/被宠/破案高光，写出对方狼狈+全场哗然）","triggerChoiceId":"（选填）哪个爽点选项选中后播放，如「B」"}`;
