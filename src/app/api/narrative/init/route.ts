...
⑨【锁定主线·选择只改姿态不改事实——最重要】原著已定的关键结局（如诸葛亮斩马谡、五丈原病逝、林冲被逼上梁山）是锁定的，无论玩家怎么选都必然发生。因此：
· 三个选项之间的差异，只能是「内心姿态 / 情感倾向 / 承受方式 / 付出的代价」层面的不同，绝不能是「改变客观事实」的不同。
· 严禁生成任何会逆转既定结局的选项，例如"连夜放走马谡使其逃脱""改判免死""临阵倒戈避免上梁山"——这类选项一律不许出现。
· 允许玩家做出恻隐、放水、犹豫、反抗等选择，但这些只代表"你此刻的姿态"，不代表事实被改写。
⑩【放水必自然收回】当玩家选择了恻隐 / 放水 / 心软 / 试图挽回类选项时，本幕后果文本必须用原著情境中本就存在的更大力量，把结局自然地拉回既定终点，并写清因果，绝不能让"放水"直接改变事实、也绝不能不给理由就硬让结局发生。可用的收回机制（择其合逻辑者）：军法/制度不容（你立的规矩反噬你）、他人意志（众将依律请斩、旁人劝阻无效）、当事人自己的选择（如马谡泣而就戮、自请领罪）、大局所迫（军心存亡压过私情）。收回后要让玩家感到：不是你没资格心软，而是"你心软了、也尽力了，但那股更大的力量仍把结局推回原点"——你付出的代价因此是内心的，而非事实的改变。`
    : ""; // 自定义书目无法保证原著锚点，略过
