...
**③ 原则的三层效力结构（宪法思想·已落码）**：
- **L1 宪法级红线**（违反=产品失去合法性）：原著诚实、价值中立、行为累积＋两次分析、**既定事实锁定**。
- **L2 核心体验支柱**（违反=体验残）：极压、锚点、两次对比、元价值轴、现代重看、抉择三件套。
- **L3 实施级手艺**（违反=局部粗糙）：其余原则。
- **冲突仲裁**：低层永远为高层让步（L1>L2>L3）。代码里 `PROHIBITIONS` 每条带 `tier`，按 L1→L2→L3 排序注入每幕 prompt。
